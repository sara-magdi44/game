document.addEventListener('DOMContentLoaded', () => {
    ...
    function startTimerAndBoxMovement(minutes) {
        ...
        function getCurrentScore() {
            ...
            const score = parseInt(scoreText.substring(7)) || 0;
            const finalscore = localStorage.getItem(`${storedUsername}_finalScore`);
            if (score >= 300) {
                const congratsPopup = new Popup({
                    id: "congrats_popup",
                    title: "Congratulations! You Win!",
                    content: `{btn-popup-button}[Home]`,
                    hideCloseButton: true,
                    disableScroll: true,
                    css:`
                    .popup.congrats_popup .popup-title{
                        font-size: 20px;
                        font-weight: 600;
                        color: #000;
                        text-align: center;
...
function endGame() {
        ...
        let score = localStorage.getItem(`${username}_finalScore`);
        const endGamePopup = new Popup({
            ...
            showImmediately: true,
            loadCallback: () => {
                const buttonExit = document.querySelector(".popup-button-exit");
                buttonExit.addEventListener('click', () => { 
                    window.location.href = 'index.html'; 
                    //if i want to close the window    window.close(); 
                });
...
class Boxes extends Object {
    ...
    constructor({ x, y, RedBox }) {
      ...
      this.RedBox = RedBox;
      this.speed = 0.2//parseInt(localStorage.getItem('speedLevel'));        
